...
M.render = function(clients, buffer_id, current_win, old_name)
    ...
    M.keybinding_autocmd(window_id, old_name, clients, buffer_id, new_buffer, position_param)
end
...
M.keybinding_autocmd = function(window_id, old_name, clients, old_buffer, new_buffer, position_param)
    ...
    for _, mode in pairs({ "i", "n", "v" }) do
        api.nvim_buf_set_keymap(new_buffer, mode, config.options.rename.key_binding.exec, "", {
            nowait = true,
            noremap = true,
            callback = function()
    ...
    -- keybinding quit
    api.nvim_buf_set_keymap(new_buffer, "n", config.options.rename.key_binding.quit, "", {
        nowait = true,
        noremap = true,
        callback = function()
            close_window(window_id)
